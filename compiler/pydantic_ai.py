"""
babel/compiler/pydantic_ai.py
─────────────────────────────
Compiles a BabelTool into a format usable with Pydantic AI.
...
Usage:
    compiled = runtime.get("com.aria.tools.weather", target="pydantic_ai")
    agent = Agent("openai:gpt-4o", tools=[compiled])

    Or access the raw wrapped function:
    compiled_fn = runtime.get("com.aria.tools.weather", target="pydantic_ai")
"""
...
class PydanticAIAdapter(BaseAdapter):
    ...
    def compile(self, tool: BabelTool) -> CompiledPydanticAITool:
        """
...
def _make_pydantic_ai_wrapper(tool: BabelTool) -> Callable:
    """
