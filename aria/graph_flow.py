...
aria/graph_flow.py
──────────────────
ARIAGraphFlow — Phase 4.

Takes the task graph produced by ARIAPlanner and executes it as a
directed AG2 multi-agent workflow.

Architecture
────────────
Each graph node becomes an AG2 AssistantAgent + UserProxyAgent pair.
Tools assigned to a node are fetched from BabelServer and wrapped in
HTTP-calling stubs so:
  - Tools run on BabelServer, not in this process
...
def _make_http_tool(
    ...
    """
    Create an AG2-compatible Python callable that executes a Babel tool
    via HTTP POST to BabelServer.

    Uses exec() to generate a function with the exact signature (name,
...
def {name}({sig_str}):
    \"\"\"{spec['description']}\"\"\"
    resp = _http_execute(_server_url, _tool_id, {{{kwargs_str}}})
    return resp
"""
...
class ARIAGraphFlow:
    ...
    def __init__(
        self,
        babel_server_url: str = "http://localhost:8765",
        llm_config: dict | None = None,
        on_event=None,
    ):
        self._server_url = babel_server_url.rstrip("/")
    ...
    def run(self, task_graph: dict, extra_env: dict[str, str] | None = None, verbose: bool = True) -> str:
        ...
        Args:
            task_graph: Dict produced by ARIAPlanner.plan()
    ...
    def _run_node(self, node: dict, context: dict[str, str], original_task: str) -> str:
        ...
        # ── System message ────────────────────────────────────────────────────
        system_msg = (
            f"You are {role}, a specialised agent in a multi-agent ARIA workflow.\n"
        ...
        for tool_id in tool_ids:
            ...
            if spec is None:
                print(f"  [WARN] Tool '{tool_id}' not found on BabelServer — skipping")
                continue
    ...
    def _warm_tool_cache(self, nodes: list[dict]) -> None:
        """Batch-fetch all tool specs needed by this graph from BabelServer."""
