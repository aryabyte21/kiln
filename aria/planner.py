  ...
  "nodes": [
    {
      ...
      "tools":          ["com.aria.tools.weather", ...]   // only tool IDs that exist
    }
  ],
...
The planner does NOT trigger Vibe synthesis — it only reports gaps.
...
PLANNER_SYSTEM = """\
You are ARIA's task planner. Your job is to decompose a user request into a \
multi-agent task graph that can be executed by a team of specialised agents, \
each armed with Babel tools fetched from a live registry.

Given:
  1. A user request
  2. A list of currently registered Babel tools (id, name, description)
...
- If a sub-task has no matching tool, describe the ideal tool in missing_tools \
  with its full id, description, inputs and output schema so Vibe Coder can synthesise it. \
...
class ARIAPlanner:
    """
    Phase 2: produces a task graph from a user request + BabelServer tool list.
    ...
    def plan(self, user_request: str, tools: list[dict] | None = None) -> dict:
        ...
        Raises:
            requests.ConnectionError  – BabelServer is not running (only when tools=None)
    ...
    def _fetch_tools(self) -> list[dict]:
        ...
        try:
            ...
            raise requests.ConnectionError(
                f"BabelServer not reachable at {self._server_url}. "
