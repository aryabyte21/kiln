"""
babel/compiler/langchain.py
───────────────────────────
Compiles a BabelTool into a LangChain StructuredTool.

LangChain expects:
    - StructuredTool with name, description, func, args_schema
    - args_schema is a Pydantic BaseModel class

We dynamically build the Pydantic model from the BabelToolSpec params,
so you get full type validation and schema generation for free.

Usage:
    compiled = runtime.get("com.aria.tools.weather", target="langchain")
...
from ..spec import BabelTool, ToolParam
...
class LangChainAdapter(BaseAdapter):
    ...
    def compile(self, tool: BabelTool) -> Any:
        """
        Returns a LangChain StructuredTool.
        The args_schema is dynamically built from the BabelToolSpec.
        """
        try:
        ...
        print(f"[Babel→LangChain] Compiled: {spec.name}")
...
def _build_pydantic_model(model_name: str, params: list[ToolParam]) -> Type:
    ...
    # Map Babel type strings → Python types
    type_map = {
        "str":   str,
        "int":   int,
        "float": float,
