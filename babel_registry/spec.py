...
babel/spec.py
─────────────
The heart of Babel. A BabelTool is a plain Python function
plus a BabelToolSpec that describes it in a framework-agnostic way.
...
@dataclass
class ToolParam:
    """One input parameter of a Babel tool."""
    name: str
...
class BabelToolSpec:
    ...
    """
    id: str                             # e.g. "com.aria.tools.weather"
...
class BabelTool:
    """
    A registered Babel tool: spec metadata + the actual callable.
...
def babel_tool(
    ...
    """
    Decorator that wraps a plain Python function as a BabelTool.
    ...
    def decorator(fn: Callable) -> BabelTool:
...
def _python_type_to_str(t: Any) -> str:
    """Map Python type → Babel type string."""
    mapping = {
        str: "str",
        int: "int",
        float: "float",
