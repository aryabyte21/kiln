...
babel/loader.py
───────────────
BabelLoader: load a tool from a spec.yaml + implementation.py on disk.

This is the entry point for Vibe-synthesised tools.
Vibe writes two files into the registry directory:

    spec.yaml          ← validated against babel.schema.json
    <entrypoint>.py    ← pure Python, no framework imports

BabelLoader reads both, validates the spec, dynamically imports the
function, runs the test fixtures, and registers the result in the
SQLite registry — producing the same BabelTool object that the
@babel_tool decorator produces for hand-written tools.
...
The pipeline:
    ...
    spec.yaml + implementation.py
          ...
          ↓  register in SQLite
          ↓
        BabelTool  →  any adapter  →  any framework

Usage:
    loader = BabelLoader()

    # Load & register one versioned tool
    tool = loader.load("registry/tools/com.aria.tools.weather/1.0.0")
...
from .spec import BabelTool, BabelToolSpec, ToolParam, ToolReturn
from .registry import register


# ── Schema ────────────────────────────────────────────────────────────────────

_SCHEMA_PATH = Path(__file__).parent / "spec" / "babel.schema.json"

# ── YAML type → Babel type string ─────────────────────────────────────────────
...
class BabelLoader:
    """
    Loads BabelTools from spec.yaml + implementation.py on disk.

    Identical output to @babel_tool — both produce a BabelTool that the
    runtime and all adapters treat exactly the same way.

    This class is what Vibe synthesis writes into and what the planner
    uses to hydrate the registry at startup.
    """
    ...
    def load(self, tool_dir: str | Path) -> BabelTool:
        """
        ...
        Args:
            tool_dir: Versioned directory, e.g.
                      registry/tools/com.aria.tools.weather/1.0.0

        Returns:
            A BabelTool registered in the SQLite registry and ready for
        ...
        tool    = BabelTool(spec=spec, fn=fn)

        if self._auto_register:
            register(tool)

        print(f"[Babel/Loader] Loaded: {tool.id}  v{tool.spec.version}  from {tool_dir}")
        return tool
    ...
    def test(self, tool_dir: str | Path) -> dict:
        ...
        Used by the Vibe synthesis loop to decide whether to publish
    ...
    def load_all(self, registry_dir: str | Path) -> list[BabelTool]:
        """
        ...
        for tool_id_dir in sorted(registry_dir.iterdir()):
            ...
            try:
                ...
                print(f"[Babel/Loader] Skipped {tool_id_dir.name}: {exc}")
    ...
    def _build_spec(self, raw: dict) -> BabelToolSpec:
        """Convert parsed YAML dict → BabelToolSpec (same type all adapters use)."""
        tool_meta = raw["tool"]
        iface     = raw["interface"]
