...
babel/runtime.py
────────────────
BabelRuntime: the main entry point for consuming tools.
...
from .registry import BabelRegistry, get_global_registry
...
class BabelRuntime:
    """
    Loads tools from a BabelRegistry and compiles them
    ...
    def __init__(
        self,
        target: str = "ag2",
        registry: BabelRegistry | None = None,
    ):
    ...
    def get(self, tool_id: str, target: str | None = None) -> Any:
        ...
        babel_tool = self._registry.get(tool_id)
        if babel_tool is None:
            babel_tool = self._registry.query(tool_id)
        if babel_tool is None:
            raise KeyError(
        ...
        return self._compile(babel_tool, target)
    ...
    def _compile(self, tool: BabelTool, target: str) -> Any:
        ...
        if cache_key not in self._cache:
            ...
            print(f"[Babel] Compiled {tool.id} → {target}")
